...
namespace AppointmentScheduling.Services
{
    public class AppointmentService : IAppointmentService
    {
        ...
        public async Task<int> AddUpdate(AppointmentVM model)
        {
            ...
            if (!DateTime.TryParseExact(model.StartDate, "M/d/yyyy h:mm tt",
        ...
        public List<AppointmentVM> DoctorsEventsById(string doctorId)
        {
            return _db.Appointments.Where(x => x.DoctorId == doctorId).ToList().Select(c => new AppointmentVM()
            {
                ...
                StartDate = c.StartDate.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss"),
                EndDate = c.EndDate.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss"),
                Title = c.Title,
                Duration = c.Duration,
